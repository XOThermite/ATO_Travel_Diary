...
const RATE_YEARS = [
  {
    ...
    capital:  { B: 33.90, L: 38.10, D: 64.95, I: 23.95 },
    regional: { B: 30.35, L: 34.65, D: 59.75, I: 23.95 },
  },
  {
    fy: "2025-26", ref: "TD 2025/4", from: "2025-07-01", to: "2026-06-30",
    capital:  { B: 34.75, L: 39.10, D: 66.65, I: 24.50 },
    regional: { B: 31.15, L: 35.55, D: 61.30, I: 24.50 },
  },
];
const rateYearFor = (dateStr) => RATE_YEARS.find(r => dateStr >= r.from && dateStr <= r.to) ?? RATE_YEARS[RATE_YEARS.length - 1];
const ratesAt = (dateStr, locType) => rateYearFor(dateStr)[locType];
...
const fyOf        = (s) => { const d = parseDate(s); const y = d.getFullYear(); return d.getMonth() >= 6 ? `${y}-${String(y + 1).slice(2)}` : `${y - 1}-${String(y).slice(2)}`; };
...
const dayAllowance = (date, expected, locType, pctOrLegacy, legacyCap) => {
  const r = ratesAt(date, locType);
...
const tripAllowance = (trip) =>
  trip.days.reduce((s, d) => s + dayAllowance(d.date, d.expected, trip.locType, tripPct(trip), trip.rateCap).allowance, 0);
...
const dayClaimable = (d, locType) => { const r = ratesAt(d.date, locType); return ITEMS.reduce((s, { key }) => s + Math.min(parseFloat(d.actual?.[key]) || 0, r[key]), 0); };
    ...
    [...trips].sort((a, b) => a.startDate.localeCompare(b.startDate)).forEach(trip => {
      trip.days.forEach(day => {
        ...
        rows.push({
          ...
          "FY":                       fyOf(day.date),
          "ATO Reference":            rateYearFor(day.date).ref,
        });
      });
    });
                  ...
                  <div style={{ display: "flex", alignItems: "center", padding: "14px 16px", gap: 12, cursor: "pointer" }}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
                        ...
                        <span style={{ fontSize: 10, color: "#B0BAC5" }}>{nd} {nd === 1 ? "day" : "days"} · {rateYearFor(t.startDate).ref} · determined {fmtLong(t.determinedOn)}</span>
                      </div>
                    ...
                    <div style={{ borderTop: "1px solid #F0EDE8", padding: "12px 16px" }}>
                      ...
                      <div style={{ overflowX: "auto", borderRadius: 8, border: "1px solid #E8E4DC" }}>
                        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 720 }}>
                          ...
                          <tbody>
                            {t.days.map((d, i) => {
                              const det = dayAllowance(d.date, d.expected, t.locType, tripPct(t), t.rateCap);
                              const edit = diaryEdit[t.id]?.find(x => x.date === d.date);
                              const act = editing
                                ? ITEMS.reduce((s, { key }) => s + (parseFloat(edit?.[key]) || 0), 0)
                                : dayActual(d);
                              const claim = editing
                                ? ITEMS.reduce((s, { key }) => s + Math.min(parseFloat(edit?.[key]) || 0, ratesAt(d.date, t.locType)[key]), 0)
                          ...
                          <tfoot>
                            <tr style={{ background: "#1A2535" }}>
                              ...
                              <td style={{ padding: "10px 12px 10px 6px", textAlign: "right", fontFamily: "monospace", fontWeight: 700, fontSize: 14, color: "#fff" }}>
                                {editing
                                  ? fmt$(diaryEdit[t.id].reduce((s, e) => s + ITEMS.reduce((x, { key }) => x + Math.min(parseFloat(e[key]) || 0, ratesAt(e.date, t.locType)[key]), 0), 0))
